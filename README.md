...
* A **minimal** (e.g. no display, no sound) Debian buster arm64 (64bit) image suitable for the Variscite var-som-mx8m-nano.
...
``` bash
scp artifacts/var-som-mx8m-nano-buster-arm64.img variscite@IP_ADDRESS:
```
...
``` bash
sudo dd if=var-som-mx8m-nano-buster-arm64.img of=/dev/mmcblk2 bs=1M
```
